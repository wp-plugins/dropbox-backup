    ...
    if (! class_exists("wpadm_wp_full_backup_dropbox") ) {
        ...
        class wpadm_wp_full_backup_dropbox extends wpadm_class  {
            ...
            public static function local_backup()
            {
                ...
                $params = array('optimize' => 1, 'limit' => 0, 'time' => @$_POST['time'], 'types' => array('db', 'files') );
                $plus_path = get_option(PREFIX_BACKUP_ . "plus-path");
                if ($plus_path !== false) {
                    $params['plus-path'] = $plus_path;
                }
                $backup = new WPAdm_Core(array('method' => "local_backup", 'params' => $params ), 'full_backup_dropbox', dirname(__FILE__));
            ...
            public static function dropboxConnect()
            {
                ...
                if (isset($_GET['app_key']) && isset($_GET['app_secret'])) {
                    ...
                    $_COOKIE['dropbox_request_token'] = $_SESSION['dropbox_request_token'];
                    ...
                    $request_token = isset($_SESSION['dropbox_request_token']) && !is_null($_SESSION['dropbox_request_token']) ? $_SESSION['dropbox_request_token'] : ( isset($_COOKIE['dropbox_request_token']) && !is_null($_COOKIE['dropbox_request_token']) ? $_COOKIE['dropbox_request_token'] : "" ) ;
                    if (!empty($request_token)) {
                        ...
                        parent::setError( langWPADM::get('Error Auth to dropbox or Session & Cookei is empty' , false) );
                    }
                    echo '<script>
                    if(window.opener){
            ...
            {      
                ...
                if ($send_to_dropbox) {
                    ...
                    $backup_local = new WPAdm_Core(array('method' => "local_backup", 'params' => $params), 'full_backup_dropbox', dirname(__FILE__));
            ...
            public static function wpadm_show_backup()
            {
                ...
                if (isset( $_SESSION['sent_response'] ) ) {
                    $sent_response = $_SESSION['sent_response'];
                    unset($_SESSION['sent_response']); 
                }
